...
describe('ObservableList', () => {
	...
	test('#find()', () => {
		...
		expect(list.find(isThree))
			.toBe(3);

		expect(isThree)
			.toHaveBeenCalledTimes(3);

		expect(isThree.mock.calls[1])
			.toEqual([2, 1, list]);
	});
	...
	test('#reduce()', () => {
		...
		expect(list.reduce(reducer))
			.toBe(6);

		expect(reducer)
			.toHaveBeenCalledTimes(2);

		expect(reducer.mock.calls[1])
			.toEqual([3, 3, 2, list]);
	});
